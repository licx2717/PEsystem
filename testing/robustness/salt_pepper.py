...
def evaluate_noise_auc(model, test_loader, device, noise_type='gaussian', noise_param=0.1):
    ...
    for batch in test_loader:
        ...
        if noise_type == 'salt_pepper':
            noisy_images = add_salt_pepper_noise(images, prob=noise_param)
        else :
...
def main():
    ...
    noise_params ={'salt_pepper': [0, 0.05, 0.1, 0.15, 0.20,0.25,0.30]} # 椒盐噪声的概率
    ...
    for noise_type in noise_types:
        ...
        # 设置坐标轴标签
        plt.xlabel('Noise probability', fontsize=12, fontweight='bold')
