...
def calculate_net_benefit_model(thresh_group, y_pred_score, y_label):
    ...
    for thresh in thresh_group:
        ...
        # 新增：打印净收益范围
        print(f"\nNet Benefit Range: [{min(net_benefit_model):.3f}, {max(net_benefit_model):.3f}]")
        print(f"Mean Net Benefit: {np.mean(net_benefit_model):.3f}")
...
if __name__ == '__main__':
    dir_name = 'D:\\MONAI-dev\\K_repeats\\DCA\\'
    ...
    output_path = os.path.join(dir_name, 'decision_curve_analysis.png')
