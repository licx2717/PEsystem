...
def read_data(file_path):
    ...
    # 使用正则表达式提取概率值
    # try:
    #     y_scores = [re.search(r'\[(\d+\.\d+)', str(score)).group(1) for score in y_scores]
...
def plot_roc_with_confidence(y_true, y_scores, set_name, ax, n_bootstrap=500, alpha=0.05):
    ...
    # 绘制 ROC 曲线
    ax.plot(fpr, tpr, label=f'{set_name} (AUC = {roc_auc:.2f})', lw=2)
    ax.fill_between(mean_fpr, tpr_lower, tpr_upper, alpha=0.3)
...
def process_excel_files(directory):
    ...
    for file_path in file_paths:
        try:
            ...
            # 绘制 ROC 曲线
            plot_roc_with_confidence(y_true, y_scores, set_name, ax)
        except Exception as e:
            print(f"Error processing file {file_path}: {e}")

    # 添加图例和标签
    ax.plot([0, 1], [0, 1], 'k--', lw=2)  # 绘制对角线
    ax.set_xlim([0.0, 1.0])
    ...
    plt.savefig('D:\\MONAI-dev\\K_repeats\\val_records\\roc_curves(tri).png', dpi=600, bbox_inches='tight')  # 保存为高分辨率图片
