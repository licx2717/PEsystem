...
from monai.transforms import (
    ...
    ScaleIntensityd, Spacingd, RandCropByPosNegLabeld, RandAffined, RandRotate, RandGaussianNoised
)
...
def main():
    ...
    Train_images_path, Train_images_label, val_images_path, val_images_label = read_split_data(train_root, val_rate=0.5)

    # 设置transforms
    data_transform = {
        "train": monai.transforms.Compose([
                                           #transforms.ToTensor(),
                                           AddChannel(),
                                           RandRotate(prob=0.5),
                                           RandFlip(prob=0.5)
    ...
    optimizer = optim.Adam(net.parameters(), lr=0,weight_decay=1e-2)
    ...
    step_lr_list = []
    # 添加tensorboard
    logdir = "D:\\ultrasound_re\\model\\model_vgg\\logs\\logs7_MODELTEST"
